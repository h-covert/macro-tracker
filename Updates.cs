...
public sealed partial class MainWindow {
    ...
    void UpdateSettingsCard() {
        ...
        var check=Button("Check for updates",delegate{},true);check.Name="CheckForUpdates";check.Width=210;check.HorizontalAlignment=HorizontalAlignment.Left;check.Click+=async delegate{await CheckUpdates(true);};p.Children.Add(check);
        var note=Label("When an update is available, choose Download update. The app checks the download, applies it, and restarts. Your diary and API key stay in their data folder.",13,Muted);note.MaxWidth=800;note.HorizontalAlignment=HorizontalAlignment.Left;p.Children.Add(note);
        AddSettingsBox(p);
    }
