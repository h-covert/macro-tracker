...
public sealed partial class MainWindow {
 void ReminderSettings(){
  ...
  if(reminders.Count>0){
   ...
   var picker=Choose(panel,"Choose a reminder",labels,labels[index]);picker.Name="ReminderPicker";
   ...
   Action<int> show=delegate(int at){
    ...
    var message=Field(editor,"Message",item["message"]);message.Name="ReminderMessage";
  ...
  var extra=Row();extra.Children.Add(Button("+ Add reminder",delegate{store.Db.Query("INSERT INTO reminders(time,message,enabled) VALUES('10:00','Time to log your food.',0)");var added=store.Db.Query("SELECT id FROM reminders ORDER BY id DESC LIMIT 1")[0]["id"];store.Set("settings-reminder",added);Navigate(page);},true));extra.Children.Add(Button("Test notification",delegate{Notify("Test reminder",store.Context("Anything to log?",DateTime.Now));Notice("Test notification sent to Windows");},true));panel.Children.Add(extra);content.Children.Add(Box(panel));
 }
