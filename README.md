...
**Library → Scan nutrition label** can take a photo with the built-in Windows Camera app, choose an image file, paste an image, or accept pasted text. Access camera opens Windows Camera; after taking the photo, return to Macro Tracker and choose **Use captured photo**. The app detects the newly saved Camera Roll image and sends it into the review. This avoids the legacy WDM/Win32 preview used in version 1.4.4, which could show a black frame on modern integrated cameras. Windows OCR runs locally through Windows PowerShell with the installed OCR language. The label parser tolerates common photo-layout issues, including calories returned on a later OCR line and a protein unit read as a digit. No internet, external AI account, or extra API key is required. Review the source beside extracted calories, protein, total carbs and total fat. Missing/ambiguous values remain blank; correct them and confirm before saving to Favorites. English labels are supported. Fiber, sugar, sodium and servings per container are saved as notes; they are not separate tracked daily nutrients. OCR can misread characters and multi-column labels: verify the per-serving basis. If the camera is unavailable, Choose image and Paste image remain available. If OCR is unavailable or blocked by local policy, the editable text review remains available.
...
- **Add food / Ctrl+N:** enter a food or select **Search USDA foods**. Choose **Item / serving**, **Grams**, or **Ounces**, then enter the amount eaten. Missing nutrients stay blank for review. Save favorites for offline reuse.
- **Amount and measure:** macros use the basis named beside each field. For two eggs, choose **Item / serving**, describe `1 whole egg`, enter per-egg macros, and set Amount to `2`. For weighed foods, choose **Grams** or **Ounces** and enter macros per one gram or one ounce. USDA household portions such as `1 large` are offered where available.
- **Fast numeric editing:** clicking or tabbing into Amount, calories, protein, carbs, or fat selects the full existing value so the next number typed replaces it.
- **Time eaten:** choose a clock time in 15-minute intervals and select AM or PM. New, edited, copied, and saved-meal entries use the same picker. The log sorts by meal time. Use the date picker for another day.
