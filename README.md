...
**Library → Scan nutrition label** can take a photo with the built-in Windows Camera app, choose an image file, paste an image, or accept pasted text. Access camera opens Windows Camera; after taking the photo, return to Macro Tracker and choose **Use captured photo**. The app detects the newly saved Camera Roll image and sends it into the review. This avoids the legacy WDM/Win32 preview used in version 1.4.4, which could show a black frame on modern integrated cameras. Windows OCR runs locally through Windows PowerShell with the installed OCR language. No internet, external AI account, or extra API key is required. Review the source beside extracted calories, protein, total carbs and total fat. Missing/ambiguous values remain blank; correct them and confirm before saving to Favorites. English labels are supported. Fiber, sugar, sodium and servings per container are saved as notes; they are not separate tracked daily nutrients. OCR can misread characters and multi-column labels: verify the per-serving basis. If the camera is unavailable, Choose image and Paste image remain available. If OCR is unavailable or blocked by local policy, the editable text review remains available.
